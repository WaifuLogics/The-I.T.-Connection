...
if (isset($_POST['type']) && $_POST['type'] != "") {
    switch ($_POST['type']) {
        case "request":
            ...
            $result = ReturnQueryResult("SELECT account_name, account_id FROM accounts WHERE account_id = :accId",
                ['accId' => $curUser], $container->database);
            if (count($result) > 0) {
        ...
        case "accept":
            ...
            $result = ReturnQueryResult("SELECT account_name, account_id FROM accounts WHERE account_id = :accId",
                                         ['accId' => $requester], $container->database);
            if (count($result) > 0) {
                foreach ($result as $data) {
                    $friendAddName = $data['account_name'];
                }
